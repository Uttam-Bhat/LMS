...
const MOCK_QUESTIONS = [
  { id: 1, question: 'What is 2 + 2?', options: ['2', '3', '4', '5'], answer: null },
  { id: 2, question: 'What is the capital of France?', options: ['London', 'Berlin', 'Paris', 'Rome'], answer: null },
  { id: 3, question: 'Which planet is known as the Red Planet?', options: ['Earth', 'Mars', 'Jupiter', 'Saturn'], answer: null },
];
  ...
  useEffect(() => {
    const fetchExam = async () => {
      ...
      try {
        ...
        // Fetch questions for this exam (replace with real API if available)
            ...
            {questions.map((q, idx) => (
              <div key={q.id} style={{ marginBottom: 28 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {q.options.map(opt => (
                    <label key={opt} style={{ fontSize: '1.05rem', color: '#374151', background: '#f6f8fb', borderRadius: 6, padding: '7px 14px', cursor: 'pointer', border: answers[q.id] === opt ? '2px solid #2563eb' : '1px solid #e5e7eb', fontWeight: answers[q.id] === opt ? 700 : 400 }}>
                      <input type="radio" name={`q_${q.id}`} value={opt} checked={answers[q.id] === opt} onChange={() => handleOptionChange(q.id, opt)} style={{ marginRight: 10 }} />
                      {opt}
                    </label>
                  ))}
                </div>
