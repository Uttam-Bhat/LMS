      ...
      <div className="dashboard-main-content" style={{ position: 'relative', minHeight: '80vh', padding: 0, margin: 0 }}>
        ...
        {showConfirm && (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.25)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 2px 16px #e0e7ef', padding: '2rem 2.5rem', minWidth: 320, textAlign: 'center' }}>
              <div style={{ fontWeight: 700, fontSize: '1.25rem', marginBottom: 18 }}>Are you sure you want to end the test?</div>
              <div style={{ display: 'flex', gap: 16, justifyContent: 'center', marginTop: 12 }}>
                <button onClick={handleConfirmEnd} style={{ background: '#2563eb', color: '#fff', border: 'none', borderRadius: 8, padding: '0.7em 2.2em', fontWeight: 600, fontSize: '1.08rem', cursor: 'pointer' }}>Yes, End Test</button>
                <button onClick={handleCancelEnd} style={{ background: '#e5e7eb', color: '#222', border: 'none', borderRadius: 8, padding: '0.7em 2.2em', fontWeight: 600, fontSize: '1.08rem', cursor: 'pointer' }}>Cancel</button>
              </div>
            </div>
          </div>
        )}
        <div style={{ maxWidth: 700, margin: '0 auto', padding: '2.5rem 0 0 0' }}>
          {result && (
            <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 16px #e0e7ef', padding: '2.5rem 3rem', textAlign: 'center', marginBottom: 32, marginTop: 16 }}>
              ...
              <button onClick={() => navigate('/student/exam')} style={{ marginTop: 24, background: '#2563eb', color: '#fff', border: 'none', borderRadius: 8, padding: '0.9em 2.5em', fontWeight: 700, fontSize: '1.13rem', cursor: 'pointer', boxShadow: '0 1px 4px rgba(30,34,90,0.10)', transition: 'background 0.18s, color 0.18s' }}>Back to Exams</button>
            </div>
          )}
          {!result && (
